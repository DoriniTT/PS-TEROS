...
I've created a complete thickness convergence example for PS-TEROS at:

**Location:** `/home/thiagotd/git/PS-TEROS/examples/convergence/thickness_convergence/`

**Files created:**
1. `run_thickness_convergence.py` - Main example script
2. `README.md` - Documentation and usage instructions

### Script: run_thickness_convergence.py
...
1. **Obelix Cluster Configuration**
   - Code: `VASP-6.5.1-idefix@obelix`
   - Potential family: `PBE`
   - Hybrid MPI+OpenMP: 4 MPI processes, 88 cores per node
   - PBS scheduler with custom commands for Skylake nodes

2. **Metal-Appropriate VASP Parameters**
   - ISMEAR=1 (Methfessel-Paxton) for metallic systems
   - ISIF=3 for bulk (full relaxation)
   - ISIF=2 for slabs (ions only)
   - ENCUT=500 eV for Au

3. **Helper Function: `print_results(pk)`**
   - Extracts results using `get_thickness_convergence_results()`
   - Prints formatted tables with convergence status
   - Shows surface energies in both J/m² and eV/Å²
   - Highlights recommended thickness
   - Compares with experimental reference data

4. **User-Friendly CLI**
   - Submit: `python run_thickness_convergence.py`
   - Get results: `python run_thickness_convergence.py <PK>`
   - Clear error messages and instructions

5. **Structure File Instructions**
   - Script checks if Au.cif exists
   - Provides code snippet to create it
   - Uses ASE to generate FCC Au structure
...
python run_thickness_convergence.py 12345
```
...
A comprehensive README.md is also included with:
- Overview and expected results
- Prerequisites and cluster configuration
- Step-by-step setup instructions
- Usage examples for submission and monitoring
...
5. **Error handling** - Comprehensive try/except with traceback
6. **Documentation** - Extensive docstrings and inline comments

The example is production-ready and follows the standardized testing procedure from CLAUDE.md.
