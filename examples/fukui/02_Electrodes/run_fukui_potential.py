#!/usr/bin/env python
"""
Fukui Potential via Electrodes Method - SnO2 (110) Surface Example

This example demonstrates the full Phase 1 + Phase 2 Fukui workflow:
...
Phase 2 (Electrodes):
    ...
    4. Applies electrodes method to compute Fukui potential
    5. Outputs LOCPOT_FUKUI.vasp for visualization

The bulk structure is fetched from Materials Project (mp-856).

Usage:
    source ~/envs/aiida/bin/activate
    python run_fukui_potential.py
...
def main():
    """Run Fukui potential (Phase 2) calculation example."""

    print("\n" + "=" * 70)
    print("FUKUI POTENTIAL VIA ELECTRODES - SnO2 (110) Surface")
    print("Phase 1 (Interpolation) + Phase 2 (Electrodes)")
    print("=" * 70)
    ...
    # Look in the Phase 1 example directory
...
#PBS -N Fukui_Potential_SnO2''',
    ...
    # DFPT-specific settings (will be merged with builder_inputs)
    # Note: LEPSILON, IBRION=8, LREAL=False are set automatically by the workflow
    dfpt_builder_inputs = {
        'parameters': {
            'incar': {
                'prec': 'Accurate',
...
#PBS -N DFPT_SnO2_bulk''',
    ...
    # Build WorkGraph with Phase 2 enabled
    print("\n6. Building Fukui WorkGraph (Phase 1 + Phase 2)...")
    wg = build_fukui_workgraph(
        ...
        # Phase 1: Interpolation
        compute_fukui=True,
        # Phase 2: Electrodes (NEW!)
        compute_fukui_potential=True,
        bulk_structure=bulk_structure,
        dfpt_builder_inputs=dfpt_builder_inputs,
        max_concurrent_jobs=5,  # 4 slab calcs + 1 DFPT can run in parallel
        name='Fukui_Potential_SnO2_110',
    )
