#!/usr/bin/env python
"""
Fukui- (Electrophilic Attack) Calculation Example
...
1. Runs 4 parallel static VASP calculations at different charge states
   (delta_N = 0.0, 0.05, 0.10, 0.15 - electrons ADDED to system)
...
The Fukui- function f-(r) identifies sites susceptible to electrophilic attack,
i.e., where electrons are removed by an electrophile (electron acceptor).

Usage:
    source ~/envs/aiida/bin/activate
    python run_fukui_minus.py
...
#PBS -N Fukui_SnO2_110_minus''',
    ...
    for delta_n in delta_n_values:
        nelect = nelect_neutral + delta_n  # Fukui-: add electrons
    ...
    pk_file = Path(__file__).parent / 'last_pk_minus.txt'
...
Export CHGCAR_FUKUI.vasp for visualization in VESTA:
    ...
    >>> with open('CHGCAR_FUKUI_minus.vasp', 'wb' if isinstance(content, bytes) else 'w') as f:
    ...     f.write(content)

Visualize in VESTA:
    - Open CHGCAR_FUKUI_minus.vasp
    - Set isosurface value to ~0.0015 a0^-3
    - Yellow regions indicate sites susceptible to electrophilic attack
      (where electrons are removed by an electrophile/electron acceptor)
""")
    print("=" * 70)
