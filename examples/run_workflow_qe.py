...
CODE_LABEL = "QE-PW-6.6@bohr"  # Adjust to your QE pw.x installation
...
MAX_FORCE = 1.0e-3          # Force convergence criterion (Ry/bohr)
MAX_STEPS = 200             # Maximum optimization steps
SYSTEM_2D = True            # Whether to use 2D boundary conditions for slabs
...
def builder_slab_relax():
    ...
    # (this will be replaced in the workflow)
    dummy_structure = StructureData(cell=[[10, 0, 0], [0, 10, 0], [0, 0, 20]])
    dummy_structure.append_atom(position=(0, 0, 0), symbols='Ag')
    ...
    if SYSTEM_2D:
        # For 2D systems (slabs)
        parameters['SYSTEM']['assume_isolated'] = 'esm'  # Use Effective Screening Medium
        parameters['SYSTEM']['esm_bc'] = 'bc3'  # vacuum-slab-vacuum boundary
...
def builder_o2_relax():
    ...
    # Set initial magnetization for O
    parameters['SYSTEM']['starting_magnetization(1)'] = 0.5  # For oxygen
    parameters['SYSTEM']['nspin'] = 2  # Spin-polarized calculation
