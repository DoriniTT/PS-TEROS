...
This method uses `pip` to install `aiida-core`, then `aiida-psteros`, and finally the `verdi presto` command to quickly set up a default profile.
...
2.  **Install `aiida-psteros`**

    Once `aiida-core` is installed, you can install `aiida-psteros`:
    ```bash
    pip install aiida-psteros
    ```
    Alternatively, if you have cloned the repository and want to install it in editable mode (e.g., for development):
    ```bash
    git clone https://github.com/your-repo/aiida-psteros.git
    cd aiida-psteros
    pip install -e .
    ```
    Replace `https://github.com/your-repo/aiida-psteros.git` with the actual repository URL.

3.  **Set up a profile**

    After installing `aiida-core` and `aiida-psteros`, you need to set up an AiiDA profile. A profile stores all your AiiDA data. The `verdi presto` command automates this for you:
    ```bash
    verdi presto
    ```
