...
PS-TEROS workflow system. All custom exceptions inherit from PsterosError
to allow catching all PS-TEROS-specific errors with a single except clause.

Example:
    >>> from psteros.core.exceptions import ValidationError
    >>> if param < 0:
...
class ValidationError(PsterosError):
...
class StructureError(PsterosError):
...
class ConvergenceError(PsterosError):
...
class ConfigurationError(PsterosError):
...
class WorkflowError(PsterosError):
