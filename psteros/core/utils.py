"""
Shared Utilities for PS-TEROS

This module contains common utility functions used across multiple PS-TEROS modules.
...
def get_logger(name: str = 'psteros') -> logging.Logger:
    """
    Get a logger instance for PS-TEROS modules.

    This provides a consistent logging interface across all PS-TEROS modules.
    The logger integrates with AiiDA's logging system and can be configured
    via standard Python logging configuration.

    Args:
        name: Logger name (default: 'psteros')

    Returns:
        Configured logger instance

    Example:
        >>> from psteros.core.utils import get_logger
...
# Create module-level logger for PS-TEROS
logger = get_logger('psteros.core')
